package com.sulake.core.utils
{
    ...
    public class MouseWheelEnabler 
    {
        ...
        public static function init(k:Stage, _arg_2:Boolean=false):void
        {
            if (!initialised)
            {
                initialised = true;
                registerListenerForMouseMove(k);
                registerJS();
            }
            useRawValues = _arg_2;
        }
        ...
        {
            stage.addEventListener(MouseEvent.MOUSE_MOVE, function (mouseMoveEvent:MouseEvent):void
            {
                currentItem = InteractiveObject(mouseMoveEvent.target);
                browserMouseEvent = MouseEvent(mouseMoveEvent);
            });
        }
        ...
        private static function handleExternalMouseEvent(rawValues:Number, parsedValues:*):void
        {
            var values:Number;
            var time:uint = getTimer();
            if (time >= (eventTimeout + lastEventTime))
            {
                if (useRawValues)
                {
                    values = rawValues;
                ...
                {
                    currentItem.dispatchEvent(new MouseEvent(MouseEvent.MOUSE_WHEEL, true, false, browserMouseEvent.localX, browserMouseEvent.localY, browserMouseEvent.relatedObject, browserMouseEvent.ctrlKey, browserMouseEvent.altKey, browserMouseEvent.shiftKey, browserMouseEvent.buttonDown, int(values)));
                }
                lastEventTime = time;
            }
        }
        ...
        public static function getBrowserInfo():BrowserInfo
        {
            ...
            if (ExternalInterface.available)
            {
                k = ExternalInterface.call("mws.getBrowserInfo");
                _local_2 = ExternalInterface.call("mws.getPlatformInfo");
                _local_3 = ExternalInterface.call("mws.getAgentInfo");
                return new BrowserInfo(k, _local_2, _local_3);
            }
            return null;
        }
    }
